...
num_folds = 10
models = []
models.append(('Arbol de Decision', DecisionTreeClassifier()))
models.append(('k Vecinos + Cercanos', KNeighborsClassifier()))
models.append(('Maquina de Soporte Vectorial', SVC(gamma='auto')))
...
resultados = evaluateModels(models, num_folds, "accuracy", x_train, y_train)
#print(resultados)


def compareResults(results):
	new_result = pd.concat([results.mean(), results.std()], axis=1)
	new_result.columns = ["Precision media", "Desviacion Estandar"]
...
def validateModels(models, X_train, Y_train, X_validation, Y_validation):
	results = {'Modelo':[], 'Precision':[], 'Error':[], 'cf_matrix':[] }
	for name, model in models:
		model.fit(X_train, Y_train)
		Y_predicted = model.predict(X_validation)
		results['Modelo'].append(name)
		results['cf_matrix'].append(confusion_matrix(y_pred=Y_predicted, y_true=Y_validation))
		results['Precision'].append(accuracy_score(y_pred=Y_predicted, y_true=Y_validation))
...
def _initIndividual(pcls, maxints):
	...
	Arguments:
		...
		maxints {[params_size]} -- [lista de maximos indices]
	Returns:
		[creator.Individual] -- [Creacion de individuo]
	"""
...
def _mutIndividual(individual, maxints, prob_mutacion):
	"""[Mutacion Individuo]
	Arguments:
		individual {[creator.Individual]} -- [Individuo de poblacion]
		maxints {[lista]} -- [lista de maximos indices]
		prob_mutacion {[float]} -- [probabilidad de mutacion del gen]
	Returns:
...
def _cxIndividual(ind1, ind2, prob_cruce, gene_type):
	...
	Arguments:
		...
		gene_type {[list]} -- [tipos de dato de los parametros, CATEGORICO o NUMERICO]
	Returns:
...
def _individual_to_params(individual, name_values):
	"""[Set de parametro segun individuo]
	Arguments:
		individual {[creator.Individual]} -- [individuo]
		name_values {[list]} -- [lista de parametros, params_data]
	Returns:
		[diccionario] -- [parametros del individuo]
	"""
...
def _evalFunction(individual, name_values, X, y, scorer, cv, uniform, fit_params,
	...
	Arguments:
		individual {[creator.Individual]} -- [Individuo]
		name_values {[list]} -- [parametros en general]
		X {[array]} -- [Input]
		y {[array]} -- [Output]
		scorer {[string]} -- [Parametro de evaluacion, precision]
		cv {[int | cross-validation]} -- [Especificacion de los folds]
		uniform {[boolean]} -- [True hace que la data se distribuya uniformemente en los folds]
		fit_params {[dict | None]} -- [parametros para estimator.fit]
	Keyword Arguments:
		verbose {integer} -- [Mensajes de descripcion] (default: {0})
...
class EvolutiveSearchCV:
	def __init__(self, estimator, params, scoring=None, cv=4,
		...
		# Individuo [list], parametros:est, FinessMax
	...
	def _fit(self, X, y, parameter_dict):
		self._cv_results = None  # Indicador de necesidad de actualizacion
		...
		# name_values = lista de parametros, gene_type = [1:categorico; 2:numerico], maxints = size(parametros)
		name_values, self.gene_type, maxints = _get_param_types_maxint(parameter_dict)
		if self.verbose:
			print("Tipos: %s, rangos: %s" % (self.gene_type, maxints))
		# registro de funcion Individuo
		toolbox.register("individual", _initIndividual, creator.Individual, maxints=maxints)
		# registro de funcion Poblacion
		toolbox.register("population", tools.initRepeat, list, toolbox.individual)
		# Paralelismo, create pool
		if not isinstance(self.n_jobs, int):
			self.n_jobs=1
		pool = Pool(self.n_jobs)
		toolbox.register("map", pool.map)
		# registro de funcion Evaluacion
		...
		# Genealogia
		hist = tools.History()
		# Decoracion de operadores de variaznza
...
class EstimatorSelection:
	"""[Clase de Estimacion Lineal]
	init: 	inicia los modelos, parametros de entrada y
...
print("Tiempo ejecucion: "+str(helperExh1.time_modelExh['DecisionTree']))
...
#helperEvo3.scoreEvo().sort_values(['.Accuracy'], ascending=False).head(10)
