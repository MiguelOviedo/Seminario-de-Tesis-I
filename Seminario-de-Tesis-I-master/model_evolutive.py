...
class EvolutiveSearchCV:
    ...
    def cv_results_(self):
        if self._cv_results is None:
            ...
            # Get individuals and indexes (unzipping values), 
            # their list of scores, and additionally the name_values for this set of parameters
    ...
    '''@property
    def best_index_(self): # No se usa?
    ...
    def fit(self, X, y):
        ...
        #_check_param_grid(self.params) # dónde se usa?
    ...
    def _fit(self, X, y, parameter_dict):
        ...
        if self.verbose:
            print('--- El modelo evoluciona en {0} posibles combinaciones ---'.format(np.prod(np.array(maxints) + 1)))
            
        pop, logbook = algorithms.eaSimple(pop, toolbox, cxpb=self.gene_crossover_prob, 
                                           mutpb=self.gene_mutation_prob, ngen=self.generations_number, 
